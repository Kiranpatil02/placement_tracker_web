...
export default function RegisterStudent() {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {
                alertSuccess('Successful', 'Student Registered Successfully!');

                setTimeout(() => {
                    router.push("/dashboard/manager/student");
                }, 1000);
    ...
    */
    ...
    const isValid = isValidEmail && isValidRollNo && isValidBatch && isValidName && isValidCGPA && isValidSection && isValidGender && isValidHigherStudies && isValidPlaced && studentDept === "CSE";
                ...
                <header className="absolute inset-x-0 top-0 z-50">
                    <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
                        ...
                        <div className="flex flex-1 justify-end space-x-1">
                            <Link replace={true} href={"dashboard/manager/student"} className="bg-[#000000] text-[#ffffff] rounded-xl p-2 items-center align-middle flex flex-row hover:bg-[#3b3b3b] ">
                                {"All Students"}
                                <span className="material-icons ml-2">badge</span>
                            </Link>
                            <Link replace={true} href={"/dashboard/manager"} className="bg-[#000000] text-[#ffffff] rounded-xl p-2 items-center align-middle flex flex-row hover:bg-[#3b3b3b] ">
                ...
                <div className="mt-32 border border-gray-300 rounded-2xl mx-auto w-11/12 sm:max-w-11/12 md:max-w-md lg:max-w-md backdrop-blur-xl bg-gray-50 mb-8">
                    <div className="mx-auto w-full sm:max-w-11/12 md:max-w-md lg:max-w-md">
                        <div className='flex flex-row justify-center'>
                            <h1 className='px-4 py-4 w-full text-2xl font-semibold text-center'>Register Student</h1>
                        </div>
